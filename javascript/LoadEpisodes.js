        ...
        .then(data => {
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (item.isfiller == false) {
                    EpisodeElement = `<div class="dropdown-list_item"><button id="${item.id}" class="ep" onclick="LoadEpisodes(\'${item.url}\', \'${item.title}\', \'${item.id}\')">${item.title}</button></div>`
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (item.isfiller === true) {
                    if (localStorage.getItem("filler-mark") == "filler-tag") {
                        EpisodeElement = `<div class="dropdown-list_item"><button id="${item.id}" class="ep" onclick="LoadEpisodes(\'${item.url}\', \'${item.title}\', \'${item.id}\')"><span class="filler-tag">فلر</span>${item.title}</button></div>`
                    }
                    else {
                        EpisodeElement = `<div class="dropdown-list_item"><button id="${item.id}" class="ep filler-ep" onclick="LoadEpisodes(\'${item.url}\', \'${item.title}\', \'${item.id}\')">${item.title}</button></div>`
                    }
                } else {
                    EpisodeElement = `<div class="dropdown-list_item"><button id="${item.id}" class="ep" onclick="LoadEpisodes(\'${item.url}\', \'${item.title}\', \'${item.id}\')">${item.title}</button></div>`
                }
...
function LoadEpisodes(url, title, id_number) {
    if (document.querySelector(".active-ep"))   
        document.querySelector('.active-ep').classList.remove('active-ep')
    document.getElementById(`${id_number}`).classList.add("active-ep")
